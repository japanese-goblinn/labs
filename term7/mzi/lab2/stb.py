...
class STB:
    ...
    def encode_block_plain(self, chunks):
        results = []
        for X in chunks:
            Y = self.encode_block(X)
            results.append(Y)
        return results

    def decode_block_plain(self, chunks):
        results = []
        for X in chunks:
            Y = self.decode_block(X)
            results.append(Y)
        return results

    def encode_block_coupled(self, chunks, S):
        results = []
        for X in chunks:
            Y = self.encode_block(X ^ S)
            results.append(Y)
            S = Y
        return results

    def decode_block_coupled(self, chunks, S):
        results = []
        for X in chunks:
            Y = self.decode_block(X) ^ S
            results.append(Y)
            S = X
        return results

    def encode_block_gamming(self, chunks, S):
        results = []
        for X in chunks:
            Y = X ^ self.L(X, self.encode_block(S))
            results.append(Y)
            S = Y
        return results

    def decode_block_gamming(self, chunks, S):
        results = []
        for X in chunks:
            Y = X ^ self.L(X, self.encode_block(S))
            results.append(Y)
    ...
    def encode_counter(self, chunks, S):
        results = []
        s = self.encode_block(S)
        for X in chunks:
    ...
    def decode_counter(self, chunks, S):
        return self.encode_counter(chunks, S)

    def encode(self, message: str, **kwargs):
    ...
    def decode(self, message: bytes, **kwargs):
