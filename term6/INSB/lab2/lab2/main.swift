...
extension String {
    ...
    var binary: Self { self.unicodeScalars.map { $0.value }.map(binaryConvert).reduce("", +) }
    
    var humanReadable: Self {
        let sep = separateOnBlocks(of: 8, this: self)
    ...
    func returnBlocks(of size: Int) -> [Self] {
        stride(from: 0, to: self.count, by: size).map { self[$0..<$0 + size] }
    }
    
    static func <<(lhs: Self, rhs: Int) -> Self { lhs[rhs..<lhs.count] + lhs[0..<rhs] }
    
    static func ^(lhs: Self, rhs: Self) -> Self {
        zip(lhs, rhs).map { Self((Int(Self($0))!) ^ Int(Self($1))!) }.reduce("", +)
    }
}

func binaryConvert(element: UInt32) -> String {
    let result = String(element, radix: 2)
    return String(repeatElement("0", count: 8 - result.count)) + result
}

func binaryConvert(element: Int) -> String {
    let result = String(element, radix: 2)
    return String(repeating: "0", count: 4 - result.count) + result
}

func separateOnBlocks(of size: Int, this string: String) -> [String] {
...
func keys(for key: String) -> [String] {
    let kkey = key.binary
...
func s(_ index: Int, bValue: String) -> String {
...
func f(_ block: String, _ key: String) -> String {
...
func encode(this string: String, using key: String) -> String {
    assert(key.count >= 8, "KEY LENGTH ERROR")
...
let string = "Im an exception Its hard to accept Because I try to be happy But then I forget"
let key = "whatthefuckmantoolongkeyitsillegal"

let encodedString = encode(this: string, using: key)
print("Encoded:", encodedString)
let decodedString = decode(this: encodedString, using: key)
